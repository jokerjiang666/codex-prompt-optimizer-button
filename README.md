...
- 上下文增强默认关闭；关闭时不读取任何本地会话文件、不执行 git 命令。
- 开启后，仅在优化时读取本机 Codex 会话日志的最近对话摘要、工作区路径与 git 摘要，并随请求发送到你自己配置的模型服务商；上下文采集完全是只读的、不写入或修改 Codex 状态。
- 日志只在 `runtime-status.log` 中记录状态码与计数，不记录上下文正文。
